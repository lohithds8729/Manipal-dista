  ...
  const fetchData = async (startDate, endDate) => {
    try {
      ...
      const response = await relayService({
        url: `/dista/fetchapilogs/success?startDate=${formattedStartDate}&endDate=${formattedEndDate}`,
        method: "GET",
    ...
    <Container className='small-font'>
      <Card className="mt-2 p-3">
        <h4 className="small-fontsize" style={{ borderBottom: "1px solid #ddd", display: "flex", justifyContent: "space-between" }}>
          <span>API Sync Success Table</span>
        ...
        <div className="scrollable-table-container">
          <Table striped bordered hover className="table-bordered small-font" style={{ width: "100%" }}>
            ...
            <tbody>
              {currentItems
                .filter(item => {
                  const fields = item.passingData.fields.reduce((acc, field) => {
                    acc[field.name] = field.inputValue;
                    return acc;
                  }, {});

                  return searchTerm === '' ||
                    fields.aid.toLowerCase().includes(searchTerm.toLowerCase()) ||
                    fields.cid.toLowerCase().includes(searchTerm.toLowerCase()) ||
                    fields.callid.toLowerCase().includes(searchTerm.toLowerCase()) ||
                    fields.ctype.toLowerCase().includes(searchTerm.toLowerCase()) ||
                    fields.campaign.toLowerCase().includes(searchTerm.toLowerCase()) ||
                    fields.source.toLowerCase().includes(searchTerm.toLowerCase()) ||
                    fields.status.toLowerCase().includes(searchTerm.toLowerCase()) ||
                    fields.talkTime.toLowerCase().includes(searchTerm.toLowerCase());
                })
                .map((item, index) => {
                  const fields = item.passingData.fields.reduce((acc, field) => {
                    acc[field.name] = field.inputValue;
                  ...
                  return (
                    <tr key={index}>
                      <td className="text-center">{fields.aid}</td>
                      <td className="text-center">{fields.cid}</td>
                      <td className="text-center">{item.attempts}</td>
                      <td className="text-center">{fields.callid}</td>
                      <td className="text-center">{fields.ctype}</td>
                      <td className="text-center">{fields.campaign}</td>
                      <td className="text-center">{fields.startTime}</td>
                      <td className="text-center">{fields.endTime}</td>
                      <td className="text-center">{fields.duration}</td>
                      <td className="text-center">{item.createdDate}</td>
                      <td className="text-center">{fields.source}</td>
                      <td className="text-center">{fields.status}</td>
                      <td className="text-center">{fields.talkTime}</td>
                    </tr>
                  );
                })}
            </tbody>
...
export default SyncSuccessRecords;
